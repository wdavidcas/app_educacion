...
namespace DAL
{
    public class ModelCategoriaCurso
    {
        /// <summary>
        /// campos de la clase
        /// </summary>
        ...
        public ModelCategoriaCurso(int pk, string nombre, string descripcion,int estado)
        {
            this.PK = pk;
            this.Nombre = nombre;
        ...
        public ModelCategoriaCurso(int pk, string nombre, string descripcion, int estado, string error) {
            this.PK = pk;
            this.Nombre = nombre;
            this.Descripcion = descripcion;
            this.Estado = estado;
    ...
    /// </summary>
    public class ControllerCategoriaCurso 
    {
        /// <summary>
        /// campos de la clase
        /// </summary>
        ...
        /// </summary>
        public ControllerCategoriaCurso()
        {
            this.listado.Clear();
            this.Error = string.Empty;
        ...
        /// <returns></returns>
        public string Insertar(ModelCategoriaCurso categoria,bool Operacion)
        {
            try
            {
                ...
                if (!Operacion)
                {
                    ...
                    query = "UPDATE categoria SET nombre=@nombre,descripcion=@descripcion,estado=@estado,user=@user WHERE id=@id";
        ...
        /// <returns></returns>
        public bool Eliminar(ModelCategoriaCurso categoria)
        {
            try
            {
        ...
        public int Count(string busqueda,int estado) {
            try
            {
                ...
                {
                    string query = "SELECT COUNT(*) FROM categoria WHERE estado=" + estado + " AND (nombre LIKE'%" + Encryption.EncryptString(busqueda) + "%' OR descripcion LIKE '%" + Encryption.DecryptString(busqueda) + "%') AND (estado<>" + (int)Estados.Tipos.Eliminado + ")";
                    return conexion.EjecutarQueryCount(query);
                }
                else
                {
                    string query="SELECT COUNT(*) FROM categoria WHERE estado="+estado+" AND estado<>"+(int)Estados.Tipos.Eliminado+"";
                    return conexion.EjecutarQueryCount(query);
                }                
            }
            catch (Exception ex) {
        ...
        /// </summary>
        /// <returns></returns>
        public List<ModelCategoriaCurso> Listar(int inicio,int paginacion,int estado)
        {
            List<ModelCategoriaCurso> lista = new List<ModelCategoriaCurso>();
            try
            {                
                ModelCategoriaCurso categoria;
                listado.Clear();                
                listado = conexion.EjecutarSelect("SELECT id,nombre,descripcion,estado FROM categoria WHERE estado="+estado+" AND estado<>" + (int) Estados.Tipos.Eliminado + " ORDER BY nombre ASC limit " + inicio + "," + paginacion + " ");

                foreach (DataRow fila in listado.Rows)
                {
                    categoria = new ModelCategoriaCurso(Convert.ToInt32(fila["id"].ToString()),Encryption.DecryptString(fila["nombre"].ToString()),Encryption.DecryptString(fila["descripcion"].ToString()),Convert.ToInt32(fila["estado"]));
                    lista.Add(categoria);
                }
                return lista;
            }
        ...
        /// <returns></returns>
        public List<ModelCategoriaCurso> Listar(int inicio, int paginacion, string busqueda, int estado)
        {
            List<ModelCategoriaCurso> lista = new List<ModelCategoriaCurso>();
            try
            {                
                ...
                listado = conexion.EjecutarSelect("SELECT id,nombre,descripcion,estado FROM categoria WHERE estado=" + estado + " AND (nombre LIKE'%" + Encryption.EncryptString(busqueda) + "%' OR descripcion LIKE '%" + Encryption.EncryptString(busqueda)+ "%') AND (estado<>"+(int)Estados.Tipos.Eliminado+") ORDER BY nombre ASC LIMIT "+paginacion+"");// LIMIT " + inicio + "," + cantidad + " ");
        ...
        public List<ModelCategoriaCurso> Listar(int PK) {
            List<ModelCategoriaCurso> lista = new List<ModelCategoriaCurso>();
            try
            {
                ModelCategoriaCurso categoria;
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT id,nombre,descripcion,estado FROM categoria WHERE id=" + PK + " AND estado<>" + (int)Estados.Tipos.Eliminado + "");
                ...
                ModelCategoriaCurso categoria = new ModelCategoriaCurso(0, string.Empty, string.Empty, 0,ex.Message.ToString());
                lista.Add(categoria);
                return lista;
            }
        }
        ...
        public List<string> Listar(string TextoBusqueda,int Estado)
        {
            ...
            try
            {
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT id,nombre,descripcion,estado FROM categoria WHERE estado="+Estado+" AND nombre LIKE '%" + Encryption.EncryptString(TextoBusqueda) + "%'");

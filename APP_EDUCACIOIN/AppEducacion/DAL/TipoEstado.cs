...
namespace DAL
{
    ...
    public class ModelTipoEstado
    {
        ...
        public ModelTipoEstado(int pk, string nombre, string descripcion, int estado)
        {
            this.PK = pk;
            this.Nombre = nombre;
        ...
        public ModelTipoEstado(int pk, string nombre, string descripcion, int estado, string error)
        {
            this.PK = pk;
            this.Nombre = nombre;
            this.Descripcion = descripcion;
    ...
    public class ControllerTipoEstado
    {
        ...
        /// <returns></returns>
        public string Insertar(ModelTipoEstado modelo, bool Operacion)
        {
            try
            {
                ...
                if (!Operacion)
                {
                    query = "UPDATE TIPOESTADO SET nombre=@nombre,descripcion=@descripcion,estado=@estado,user=@user WHERE idTipoEstado=@id";
                    MySqlParameter paramId = new MySqlParameter("id", modelo.PK);
                    parametros.Add(paramId);
                }
                else
                {
                    query = "INSERT INTO TIPOESTADO(nombre,descripcion,estado,user) VALUES(@nombre,@descripcion,@estado,@user)";
                }
        ...
        /// <returns></returns>
        public bool Eliminar(ModelTipoEstado modelo)
        {
            try
            {
                string query = "DELETE FROM TIPOESTADO WHERE idTipoEstado=@id";
        ...
        public int Count(string busqueda, int estado)
        {
            try
            {
                ...
                {
                    string query = "SELECT COUNT(*) FROM TIPOESTADO WHERE estado=" + estado + " AND (nombre LIKE'%" + Encryption.EncryptString(busqueda) + "%' OR descripcion LIKE '%" + Encryption.EncryptString(busqueda) + "%') AND (estado<>" + (int)Estados.Tipos.Eliminado + ")";
                    return conexion.EjecutarQueryCount(query);
                }
                else
                {
                    string query = "SELECT COUNT(*) FROM TIPOESTADO WHERE estado=" + estado + " AND estado<>" + (int)Estados.Tipos.Eliminado + "";
                    return conexion.EjecutarQueryCount(query);
                }
            }
            catch (Exception ex)
        ...
        public int Count(string nombre)
        {
            try
            {
                string query = "SELECT COUNT(*) FROM TIPOESTADO WHERE nombre='" + Encryption.EncryptString(nombre.ToUpper()) + "'";
                return conexion.EjecutarQueryCount(query);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public List<ModelTipoEstado> Listar(int inicio, int paginacion, int estado)
        {
            List<ModelTipoEstado> lista = new List<ModelTipoEstado>();
            try
            {
                ModelTipoEstado modelo;
                listado.Clear();
                listado = conexion.EjecutarSelect("SELECT idTipoEstado as id,nombre,descripcion,estado FROM TIPOESTADO WHERE estado=" + estado + " AND estado<>" + (int)Estados.Tipos.Eliminado + " ORDER BY nombre ASC limit " + inicio + "," + paginacion + " ");

                foreach (DataRow fila in listado.Rows)
                {
                    modelo = new ModelTipoEstado(Convert.ToInt32(fila["id"].ToString()), Encryption.DecryptString(fila["nombre"].ToString()), Encryption.DecryptString(fila["descripcion"].ToString()), Convert.ToInt32(fila["estado"]));
                    lista.Add(modelo);
                }
                return lista;
            }
            ...
            {
                lista.Add(new ModelTipoEstado(0, ex.Message.ToString(), ex.Message.ToString(), 0, ex.Message.ToString()));
                return lista;
            }
        }
        ...
        /// <returns></returns>
        public List<ModelTipoEstado> Listar(int inicio, int paginacion, string busqueda, int estado)
        {
            List<ModelTipoEstado> lista = new List<ModelTipoEstado>();
            try
            {
                ...
                listado = conexion.EjecutarSelect("SELECT idTipoEstado as id,nombre,descripcion,estado FROM TIPOESTADO WHERE estado=" + estado + " AND (nombre LIKE'%" + Encryption.EncryptString(busqueda) + "%' OR descripcion LIKE '%" + Encryption.EncryptString(busqueda) + "%') AND (estado<>" + (int)Estados.Tipos.Eliminado + ") ORDER BY nombre ASC LIMIT " + paginacion + "");// LIMIT " + inicio + "," + cantidad + " ");
        ...
        public List<ModelTipoEstado> Listar(int estado, string orden, string campoOrden)
        {
            List<ModelTipoEstado> lista = new List<ModelTipoEstado>();
            try
            {
                ...
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT idTipoEstado as id,nombre,descripcion,estado FROM TIPOESTADO WHERE estado=" + estado + " ORDER BY '" + campoOrden + "' '" + orden + "'");
        ...
        public List<ModelTipoEstado> Listar(int PK)
        {
            List<ModelTipoEstado> lista = new List<ModelTipoEstado>();
            try
            {
                ...
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT idTipoEstado as id,nombre,descripcion,estado FROM TIPOESTADO WHERE idTipoEstado=" + PK + " AND estado<>" + (int)Estados.Tipos.Eliminado + "");
            ...
            {
                ModelTipoEstado modelo = new ModelTipoEstado(0, string.Empty, string.Empty, 0, ex.Message.ToString());
                lista.Add(modelo);
                return lista;
            }
        }
        ...
        public List<string> Listar(string TextoBusqueda, int Estado)
        {
            ...
            try
            {
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT id as idTipoEstado as id,nombre,descripcion,estado FROM TIPOESTADO WHERE estado=" + Estado + " AND nombre LIKE '%" + Encryption.EncryptString(TextoBusqueda) + "%'");

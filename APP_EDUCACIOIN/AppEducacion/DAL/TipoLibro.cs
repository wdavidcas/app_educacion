...
namespace DAL
{
    ...
    public class ModelTiposLibros
    {
        ...
        public ModelTiposLibros(int pk, string nombre, string descripcion, int estado)
        {
            this.PK = pk;
            this.Nombre = nombre;
        ...
        public ModelTiposLibros(int pk, string nombre, string descripcion, int estado, string error)
        {
            this.PK = pk;
            this.Nombre = nombre;
            this.Descripcion = descripcion;
    ...
    /// </summary>
    public class ControllerTiposLibros
    {
        /// <summary>
        /// campos de la clase
        /// </summary>
        ...
        /// <returns></returns>
        public string Insertar(ModelTiposLibros modelo, bool Operacion)
        {
            try
            {
                ...
                if (!Operacion)
                {
                    query = "UPDATE TIPOLIBROS SET nombre=@nombre,descripcion=@descripcion,estado=@estado,user=@user WHERE idTIPOLIBRO=@id";
                    MySqlParameter paramId = new MySqlParameter("id", modelo.PK);
                    parametros.Add(paramId);
                }
                else
                {
                    query = "INSERT INTO TIPOLIBROS(nombre,descripcion,estado,user) VALUES(@nombre,@descripcion,@estado,@user)";
                }
        ...
        public bool Eliminar(ModelTiposTelefono modelo)
        {
            try
            {
                string query = "DELETE FROM TIPOLIBROS WHERE idTIPOLIBRO=@id";
        ...
        public int Count(string busqueda, int estado)
        {
            try
            {
                ...
                {
                    string query = "SELECT COUNT(*) FROM TIPOLIBROS WHERE estado=" + estado + " AND (nombre LIKE'%" + Encryption.EncryptString(busqueda) + "%' OR descripcion LIKE '%" + Encryption.EncryptString(busqueda) + "%') AND (estado<>" + (int)Estados.Tipos.Eliminado + ")";
                    return conexion.EjecutarQueryCount(query);
                }
                else
                {
                    string query = "SELECT COUNT(*) FROM TIPOLIBROS WHERE estado=" + estado + " AND estado<>" + (int)Estados.Tipos.Eliminado + "";
                    return conexion.EjecutarQueryCount(query);
                }
            }
            catch (Exception ex)
        ...
        public int Count(string nombre)
        {
            try
            {
                string query = "SELECT COUNT(*) FROM TIPOLIBROS WHERE nombre='" + Encryption.EncryptString(nombre.ToUpper()) + "'";
                return conexion.EjecutarQueryCount(query);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public List<ModelTiposLibros> Listar(int inicio, int paginacion, int estado)
        {
            List<ModelTiposLibros> lista = new List<ModelTiposLibros>();
            try
            {
                ModelTiposLibros modelo;
                listado.Clear();
                listado = conexion.EjecutarSelect("SELECT idTIPOLIBRO as id,nombre,descripcion,estado FROM TIPOLIBROS WHERE estado=" + estado + " AND estado<>" + (int)Estados.Tipos.Eliminado + " ORDER BY nombre ASC limit " + inicio + "," + paginacion + " ");

                foreach (DataRow fila in listado.Rows)
                {
                    modelo = new ModelTiposLibros(Convert.ToInt32(fila["id"].ToString()), Encryption.DecryptString(fila["nombre"].ToString()), Encryption.DecryptString(fila["descripcion"].ToString()), Convert.ToInt32(fila["estado"]));
                    lista.Add(modelo);
                }
                return lista;
            }
            ...
            {
                lista.Add(new ModelTiposLibros(0, ex.Message.ToString(), ex.Message.ToString(), 0, ex.Message.ToString()));
                return lista;
            }
        }
        ...
        /// <returns></returns>
        public List<ModelTiposLibros> Listar(int inicio, int paginacion, string busqueda, int estado)
        {
            List<ModelTiposLibros> lista = new List<ModelTiposLibros>();
            try
            {
                ...
                listado = conexion.EjecutarSelect("SELECT idTIPOLIBRO as id,nombre,descripcion,estado FROM TIPOLIBROS WHERE estado=" + estado + " AND (nombre LIKE'%" + Encryption.EncryptString(busqueda) + "%' OR descripcion LIKE '%" + Encryption.EncryptString(busqueda) + "%') AND (estado<>" + (int)Estados.Tipos.Eliminado + ") ORDER BY nombre ASC LIMIT " + paginacion + "");// LIMIT " + inicio + "," + cantidad + " ");
        ...
        public List<ModelTiposLibros> Listar(int estado, string orden, string campoOrden)
        {
            List<ModelTiposLibros> lista = new List<ModelTiposLibros>();
            try
            {
                ...
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT idTIPOLIBRO,nombre,descripcion,estado FROM TIPOLIBROS WHERE estado=" + estado + " ORDER BY '" + campoOrden + "' '" + orden + "'");
        ...
        public List<ModelTiposLibros> Listar(int PK)
        {
            List<ModelTiposLibros> lista = new List<ModelTiposLibros>();
            try
            {
                ...
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT idTIPOLIBRO,nombre,descripcion,estado FROM TIPOLIBROS WHERE id=" + PK + " AND estado<>" + (int)Estados.Tipos.Eliminado + "");
            ...
            {
                ModelTiposLibros modelo = new ModelTiposLibros(0, string.Empty, string.Empty, 0, ex.Message.ToString());
                lista.Add(modelo);
                return lista;
            }
        }
        ...
        public List<string> Listar(string TextoBusqueda, int Estado)
        {
            ...
            try
            {
                ModelCredenciales credenciales = new ModelCredenciales();
                Conexion conexion = new Conexion(credenciales);
                listado = conexion.EjecutarSelect("SELECT idTIPOLIBRO as id,nombre,descripcion,estado FROM TIPOLIBROS WHERE estado=" + Estado + " AND nombre LIKE '%" + Encryption.EncryptString(TextoBusqueda) + "%'");

...
$(window).load(function () {
    ...
    headColumnas.push(["NOMBRE", "DESCRIPCION", "TIPO ESTADO", "EDITAR", "VER", "BORRAR"]);
    ...
    //crearDropDrownLIst(parametros, "Nivel.aspx/ObtenerCategoriasNivel", "POST", "ddlCategorias", "PK", "Nombre");
    getTiposEstado();

    //*************************PETICIONES AJAX****************************************
    //funcion para llenar el combo box
    function getTiposEstado() {
        var parametros = { "Estado": 1, "Orden": "ASC", "CampoOrden": "Nombre" };
        $.ajax({
            type: "POST",
            url: "Estados.aspx/ObtenerTiposEstado",
            ...
            success: function (response) {
                ...
                var combo = document.getElementById("ddlTiposEstado");
    ...
    function getData(filaInicial, cantidad) {
        ...
        $.ajax({
            type: "POST",
            url: "Estados.aspx/ObtenerListado",
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (listado.length > 0) {
                    ...
                    var tipoEstado = listado[0].TipoEstado_Id;

                    $("#txtNombre").val(nombre);
                    $("#txtDescripcion").val(descripcion);
                    $("#ddlEstado").val(estado);
                    $("#ddlTiposEstados").val(tipoEstado);
                    $("#pk").val(id);
                    ...
                    if (esGuardar == null) {
                        ...
                        $("#ddlTiposEstados").attr("disabled", -1);
                    }
                    else {
                        $("#txtNombre").removeAttr("disabled");
                        $("#txtDescripcion").removeAttr("disabled");
                        $("#ddlEstado").removeAttr("disabled");
                        $("#ddlTiposEstados").removeAttr("disabled");
                    }
                }
                else {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (listado.length > 0) {
                    ...
                    var descripcion = listado[0].Descripcion;
                    var estado = listado[0].Estado;
                    var categoria = listado[0].TipoEstado_Id;
    ...
    function crearTabla(response) {
        ...
        if (listado.length > 0) {
            ...
            for (var i = 0; i < listado.length; i++) {
                ...
                for (var j = 0; j < columnCount; j++) {
                    ...
                    if (j == 3) {
                        ...
                        cell.innerHTML = listado[i].TipoEstadoNombre;
                    }
                }
            }
            ...
            dvTable.className = "errorEdificio";
        }
    }
    ...
    $('#formulario').dialog({
        ...
        buttons: {
            ...
            "Aceptar": function () {
                ...
                var categoria = $("select[id=ddlCategorias]").val();
                llave = $("#pk").val();
